var logbook_controller = {
    ...
    restoreDiveScore: function () {
        for (var i = 1; i <= logbook_model.scoreInt; i++) {
            $('#diveScore_' + i).attr('src', '/i/heart-full.png');
        }
        for (i = logbook_model.scoreInt + 1; i <= 5; i++) {
    ...
    setDiveScore: function (elemId) {
        var scoreInt = parseInt(elemId.split('_')[1]);
        logbook_model.scoreInt = scoreInt;
        switch (scoreInt) {
            case 1:
                logbook_model.score = 'ONE_STAR';
                break;
            case 2:
                logbook_model.score = 'TWO_STAR';
                break;
            case 3:
                logbook_model.score = 'THREE_STAR';
                break;
            case 4:
                logbook_model.score = 'FOUR_STAR';
                break;
            case 5:
                logbook_model.score = 'FIVE_STAR';
                break;
        }
    ...
    createLogbookEntry: function () {
        ...
        var createForm = {
            spotId: logbook_model.spotId,
            duration: $('#duration').val(),
            depth: $('#depth').val(),
            diveDate: $('#diveDate').val(),
            score: logbook_model.score,
            visibility: $('#visibility').val(),
            note: $('#comments').val(),
            instructorId: logbook_model.instructorId,
            buddiesIds: logbook_model.buddiesIds,
            photo: imageData
        };
